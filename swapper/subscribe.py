...
from swapper.helpers import order_is_close_to_be_filled
...
async def subscribe(websocket: websockets.WebSocketClientProtocol) -> None:
    ...
    while True:
        ...
        calculated_bid_price = calculate_bid_price_based_on_spread(low_price, spread)
        calculated_ask_price = calculate_ask_price_based_on_spread(high_price, spread)
        ...
        if not state.has_active_orders():
            initial_orders_placement = await asyncio.gather(
                *[place_order(SIDE_BID, calculated_bid_price),
                  place_order(SIDE_ASK, calculated_ask_price)]
            )
            logger.info(
                f"Placed BID order: {initial_orders_placement[0]['orderId']} "
                f"with {calculated_bid_price} price"
                f"Placed ASK order: {initial_orders_placement[0]['orderId']} "
                f"with {calculated_ask_price} price"
            )
        else:
            # There are active orders. Check if they need to be cancelled and replaced
            bid_order = state.get_active_bid_order()
            if bid_order and order_is_close_to_be_filled(calculated_bid_price, calculated_ask_price,
                                                         bid_order):
                logger.warning(
                ...
                new_order = await place_order(SIDE_BID, calculated_bid_price)
                logger.info(
                    f"Placed BID order: {new_order['orderId']} with {calculated_bid_price} price"
                )
            elif not bid_order:
                new_order = await place_order(SIDE_BID, calculated_bid_price)
                logger.info(
            ...
            if ask_order and order_is_close_to_be_filled(calculated_bid_price, calculated_ask_price,
                                                         ask_order):
                logger.warning(
                ...
                new_order = await place_order(SIDE_ASK, calculated_ask_price)
                logger.info(
                    f"Placed ASK order: {new_order['orderId']} with {calculated_ask_price} price"
                )
            elif not ask_order:
                new_order = await place_order(SIDE_ASK, calculated_ask_price)
                logger.info(
